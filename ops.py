...
class batch_norm(tf.keras.layers.Layer):
    def __init__(self, epsilon=1e-5, momentum=0.9, name="batch_norm", **kwargs):
        ...
        self.batch_norm = tf.keras.layers.BatchNormalization(momentum=self.momentum, epsilon=self.epsilon)
...
def masking(input, label_col, attrib_num):
    ...
    temp = tf.compat.v1.where(inp_mask, temp, tf.zeros_like(temp))
